...
import { IconChevronLeft, IconChevronRight, IconInfo, IconMail, IconPen } from "@/components/ui-hubbly/icons";
import { saveStep } from "@/lib/mail/api";
...
export function SequenceStage({
  ...
  audienceSource: keyof typeof sourceLabel;
  audienceCount: number;
  audienceExcluded: number;
  onEditAudience: () => void;
  ...
  return (
    <div className="h-full min-h-0 grid grid-cols-[330px_minmax(0,1fr)_380px]">
      {/* Sequence */}
      ...
      <section aria-label="Email editor" className="px-7 py-5 flex flex-col gap-3.5 min-h-0">
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-surface border border-line">
          <SourceTag tone="accent">{sourceLabel[audienceSource]}</SourceTag>
          <div className="flex-1 min-w-0 text-[13.5px] truncate">
            <span className="font-semibold">{fmt.n(audienceCount)} contacts</span>
            <span className="text-muted">
              {" "}
              · {audienceLabel} · {fmt.n(audienceExcluded)} excluded (customers, unsubscribed)
            </span>
          </div>
          <button type="button" onClick={onEditAudience} className="text-[13.5px] font-semibold no-underline whitespace-nowrap text-accent cursor-pointer bg-transparent border-0">
            Edit audience
          </button>
        </div>
